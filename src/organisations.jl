"""
    Operator_name(operatorref; inc_file_needle = r"_shared_data")
    Operator_name(operatorref::String, r::EzXML.Node)
    Operator_name(servicejourney::EzXML.Node)
        ---> EzXML.Node
    Operator_name(servicejourneys::Vector{EzXML.Node}; inc_file_needle = r"_shared_data")
        ---> Vector{EzXML.Node}
...
# Example
```
julia> using StopsAndTimetables: Operator_name, nodecontent

julia> Operator_name("MOR:Operator:F1") |> nodecontent
"Fjord1"
```
"""
function Operator_name(operatorref; inc_file_needle = r"_shared_data")
...
function Operator_name(servicejourneys::Vector{EzXML.Node}; inc_file_needle = r"_shared_data")
    rs = roots(;inc_file_needle)
    @assert length(rs) == 1
    xp = """x:OperatorRef/@ref"""
